import { Store, Promotion, Category } from '../types';
...
export const getStores = async (): Promise<Store[]> => {
  ...
  try {
    ...
    if (e.code === 'permission-denied') {
      console.warn("⚠️ ERRO DE PERMISSÃO: O Firestore bloqueou a leitura das lojas.");
    } else {
...
export const saveStore = async (store: Partial<Store>) => {
  ...
  try {
    ...
    if (e.code === 'permission-denied') {
      throw new Error("⚠️ PERMISSÃO NEGADA: O Firestore bloqueou a gravação.");
    }
...
export const deleteStore = async (id: string) => {
  ...
  try {
    ...
    if (e.code === 'permission-denied') {
      ...
      throw new Error("⚠️ PERMISSÃO NEGADA: O Firestore bloqueou a exclusão.");
    }
...
export const deleteAllStores = async () => {
  ...
  try {
    ...
    // 3. Deletar todos os usuários (exceto o admin principal)
    const usersCol = collection(db, 'users');
...
export const getPromotionById = async (id: string): Promise<Promotion | null> => {
  ...
  try {
    ...
    return snap.exists() ? { id: snap.id, ...snap.data() } as Promotion : null;
  } catch (e: any) {
    if (e.code !== 'permission-denied') console.error("Erro ao buscar promoção:", e);
    return null;
  }
};
